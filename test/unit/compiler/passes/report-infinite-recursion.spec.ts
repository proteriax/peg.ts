...
describe("compiler pass |reportInfiniteRecursion|", function () {
  ...
  it("reports indirect left recursion", function () {
    expect(pass).to.reportError(["start = stop", "stop = start"].join("\n"), {
      message:
        "Possible infinite loop when parsing (left recursion: start -> stop -> start).",
      location: {
  ...
  describe("in sequences", function () {
    ...
    it("computes expressions that always consume input on success correctly", function () {
      expect(pass).to.reportError(["start = a start", "a 'a' = ''"].join("\n"))
      ...
      expect(pass).to.reportError(["start = a start", "a = ''"].join("\n"))
